...
if __name__ == "__main__":
	...
	file_prefix = FP[fr]
	file_split = file_prefix[file_prefix.index('.' + suffix)+6:]
	file_prefix = file_prefix[file_prefix.rfind('/')+1:file_prefix.index('.' + suffix)]
	hashobject = Fastq_Reader(inputdir,outputdir, fastq = fastq)
	f = open(hashobject.input_path+file_prefix+'.'+ suffix + file_split,'r')
	...
	while A != None:
		try:
			A,B = hashobject.generator_to_bins(hashobject.read_generator(f,max_reads=500,verbose_ids=True),rc=do_reverse_compliment)
